...
class WordTemplateGenerator:
    ...
    def _add_title_page(self):
        ...
        title_run = title.add_run('IvorySQL ' + self._l['pg_title'].replace('PostgreSQL ', '').replace('IvorySQL ', ''))
...
class getData(object):
    ...
    def checkdb(self, sqlfile=''):
        ...
        try:
            from modules.inspection.slow_query import PGSlowQueryAnalyzer
            if self.conn_db2:
        ...
        try:
            from modules.inspection.config_baseline import check_pg_config_baseline
            if self.conn_db2:
        ...
        try:
            from modules.inspection.index_health import analyze_pg_indexes
            if self.conn_db2:
