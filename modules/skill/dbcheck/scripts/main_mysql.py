...
class getData(object):
    ...
    def checkdb(self, sqlfile=''):
        ...
        try:
            # 使用增强智能分析模块（15+ 条规则）
            try:
                from modules.inspection.analyzer import smart_analyze_mysql
        ...
        try:
            from modules.inspection.slow_query import get_slow_query_analyzer, MySQLSlowQueryAnalyzer
            if self.conn_db2:
        ...
        try:
            from modules.inspection.config_baseline import check_mysql_config_baseline
            if self.conn_db2:
        ...
        try:
            from modules.inspection.index_health import analyze_mysql_indexes
            if self.conn_db2:
