...
def _get_default_baselines():
    ...
    return [
        ...
        # ── GBase 8s ──────────────────────────────────────────
        {
            'db_type': 'gbase', 'param_name': 'MAXCONNECTIONS',
            'query_sql': "SELECT cf_name, cf_effective FROM sysconfig WHERE cf_name = 'MAXCONNECTIONS';",
            'operator': '>=', 'expected_value': '200',
            'risk_level': 'MEDIUM',
            'description_zh': '最大连接数，建议不低于 200',
            'description_en': 'Max connections, recommended >= 200',
        ...
        {
            'db_type': 'gbase', 'param_name': 'SHMVIRTSIZE',
            'query_sql': "SELECT cf_name, cf_effective FROM sysconfig WHERE cf_name = 'SHMVIRTSIZE';",
            'operator': '>=', 'expected_value': '1073741824',
            'risk_level': 'MEDIUM',
            'description_zh': '共享内存虚拟段大小（字节），建议不低于 1GB',
            'description_en': 'Shared memory virtual size (bytes), recommended >= 1GB',
        },
        {
            'db_type': 'gbase', 'param_name': 'BUFFERS',
            'query_sql': "SELECT cf_name, cf_effective FROM sysconfig WHERE cf_name = 'BUFFERS';",
            'operator': '>=', 'expected_value': '100000',
            'risk_level': 'MEDIUM',
            'description_zh': '缓冲区数，建议不低于 100000',
            'description_en': 'Number of buffers, recommended >= 100000',
        },
        {
            'db_type': 'gbase', 'param_name': 'LOGFILES',
            'query_sql': "SELECT cf_name, cf_effective FROM sysconfig WHERE cf_name = 'LOGFILES';",
            'operator': '>=', 'expected_value': '6',
            'risk_level': 'HIGH',
            'description_zh': '逻辑日志文件数，应 >= 6（高可用要求）',
            'description_en': 'Logical log files, should be >= 6 for high availability',
        },
        # ── MongoDB ──────────────────────────────────────────
        {
            'db_type': 'mongodb', 'param_name': 'authenticationMechanisms',
            'query_sql': "db.runCommand({getParameter: 1, authenticationMechanisms: 1});",
            'operator': 'LIKE', 'expected_value': 'SCRAM-SHA-256',
            'risk_level': 'HIGH',
            'description_zh': '认证机制应包含 SCRAM-SHA-256（避免不安全的 MONGODB-CR）',
            'description_en': 'Authentication mechanisms should include SCRAM-SHA-256',
        },
        {
            'db_type': 'mongodb', 'param_name': 'enableLocalhostAuthBypass',
            'query_sql': "db.runCommand({getParameter: 1, enableLocalhostAuthBypass: 1});",
            'operator': '=', 'expected_value': 'false',
            'risk_level': 'HIGH',
            'description_zh': '应禁用 localhost 认证绕过（防止未授权访问）',
            'description_en': 'localhost auth bypass should be disabled',
        },
        {
            'db_type': 'mongodb', 'param_name': 'maxConnections',
            'query_sql': "db.serverStatus().connections.available;",
            'operator': '>=', 'expected_value': '500',
            'risk_level': 'MEDIUM',
            'description_zh': '最大连接数，建议不低于 500',
            'description_en': 'Max connections, recommended >= 500',
        },
        {
            'db_type': 'mongodb', 'param_name': 'wiredTigerCacheSizeGB',
            'query_sql': "db.serverStatus().wiredTiger.cache['maximum bytes configured'];",
            'operator': '>=', 'expected_value': '1073741824',
            'risk_level': 'MEDIUM',
            'description_zh': 'WiredTiger 缓存大小（字节），建议不低于 1GB',
            'description_en': 'WiredTiger cache size (bytes), recommended >= 1GB',
        },
        {
            'db_type': 'mongodb', 'param_name': 'writeConcernMajorityJournalDefault',
            'query_sql': "db.runCommand({getParameter: 1, writeConcernMajorityJournalDefault: 1});",
            'operator': '=', 'expected_value': 'true',
            'risk_level': 'HIGH',
            'description_zh': 'majority write concern 应等待 journal 落盘',
            'description_en': 'writeConcernMajorityJournalDefault should be true',
        },
        {
            'db_type': 'mongodb', 'param_name': 'slowOpThresholdMs',
            'query_sql': "db.runCommand({getParameter: 1, slowOpThresholdMs: 1});",
            'operator': '<=', 'expected_value': '100',
            'risk_level': 'LOW',
            'description_zh': '慢查询阈值（毫秒），建议不超过 100ms',
            'description_en': 'Slow operation threshold (ms), recommended <= 100',
        },
    ]
