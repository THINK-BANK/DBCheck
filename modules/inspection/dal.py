...
def _ensure_tables(conn):
    ...
    if baseline_count == 0:
        ...
        # 延迟导入，避免循环依赖
        try:
            from modules.inspection.init_db import init_default_templates, init_server_thresholds
...
def get_chapters_by_template(template_id: int, db_path: str = None,
                              chapter_ids: Optional[List[int]] = None) -> List[Dict]:
    """
    获取指定模板的所有章节（按 sort_order 排序）。

    :param template_id: 模板 ID
    :param db_path: 数据库文件路径
    :param chapter_ids: 巡检「章节可选」参数。
                        None / 不传 -> 全量（等价旧行为，不过滤）；
                        非空 list   -> 仅返回选中章节（参数化 IN 过滤）；
                        空 list     -> 非法输入（上层应已 400 拦截），兜底返回全量。
    :return: 章节信息字典列表
    """
    conn = get_db_connection(db_path)
    cursor = conn.cursor()

    # 章节可选过滤：复用共享 helper（逻辑单点，参数化占位符 ?）
    try:
        from modules.inspection.chapter_filter import build_chapter_filter_sql
    except Exception:
        # inspection 包不可用时（极少见）退化为不过滤，保证兼容
        build_chapter_filter_sql = lambda ids, alias='c': ('', [])

    try:
        _chap_frag, _chap_params = build_chapter_filter_sql(chapter_ids, alias='c')
        cursor.execute("""
            SELECT c.*,
                   COUNT(q.id) as query_count
            FROM inspection_chapter c
            LEFT JOIN inspection_query q ON c.id = q.chapter_id AND q.enabled = 1
            WHERE c.template_id = ?""" + _chap_frag + """
            GROUP BY c.id
