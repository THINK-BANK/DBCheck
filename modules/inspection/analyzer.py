...
class HistoryManager:
    ...
    def __init__(self, base_dir: str):
        ...
        # 避免直接继承导致的 MRO 问题
        try:
            from modules.inspection.db_history import SQLiteHistoryManager
...
class AIAdvisor:
    ...
    def _init_rag(self, rag_config: dict = None):
        """初始化 RAG 知识库（失败时静默降级）"""
        try:
            from modules.rag import RAGRetriever, VectorStore, OllamaEmbedding
            vs = VectorStore()
            ...
            if self.backend == 'openai':
                ...
                from modules.rag import OpenAIEmbedding
                emb = OpenAIEmbedding(
...
def smart_analyze_tidb(context: dict) -> list:
    ...
    # ── 3. 数据库用户安全 ──
    users = context.get('mysql_users', [])
    # mysql_users 查询返回列名：col1=用户, col2=主机, col7=是否设置密码(1/0)
    _SYS_USERS = ('mysql.sys', 'mysql.session', 'mysql.infoschema')
    for u in users:
        uname = str(u.get('col1') or u.get('User') or '')
        host = str(u.get('col2') or u.get('Host') or '')
    ...
    for disk in context.get('system_info', {}).get('disk_list', []):
        usage = _float(disk.get('usage_percent', 0))
        mp = disk.get('mountpoint', '/')
        from modules.inspection.analyzer import IGNORE_MOUNTS
        if mp in IGNORE_MOUNTS:
            continue
        if usage > 90:
            issues.append({
...
def smart_analyze_kingbase(context: dict) -> list:
    """
    对 KingbaseES 巡检结果执行风险规则分析。
    KingbaseES 兼容 PostgreSQL 协议，复用 PG 规则。
    """
    # 复用 PostgreSQL 的风险规则
    return smart_analyze_pg(context)
