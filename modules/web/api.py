...
def _get_version():
    try:
        with open(os.path.join(str(PROJECT_ROOT), 'modules', 'config', 'version.py'), 'r', encoding='utf-8') as f:
            for line in f:
...
def _ensure_plugin_type(p: dict, base_dir: str) -> str:
    ...
    try:
        from modules.pluginkit.type import detect_plugin_type
    except Exception:
...
def api_uninstall_plugin(plugin_id):
    ...
    try:
        ...
        base_dir = os.path.join(str(PROJECT_ROOT), 'plugins')
