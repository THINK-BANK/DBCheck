...
用法：
    from modules.pluginkit.market import PluginMarket
    market = PluginMarket()
...
# 内置 registry.json 的文件路径（统一收口到 config/，见 core.paths）
BUILTIN_REGISTRY_FILE = paths.BUILTIN_REGISTRY_JSON
...
class PluginMarket:
    ...
    def _load_builtin_registry(self) -> Optional[Dict]:
        ...
        try:
            builtin_path = os.path.join(
                str(PROJECT_ROOT), BUILTIN_REGISTRY_FILE
            )
    ...
    def install(self, plugin_id: str) -> Dict:
        ...
        if os.path.isdir(available_dir) and not os.path.isdir(enabled_dir):
            try:
                ...
                from modules.pluginkit.core import load_plugin_with_error, PluginRegistry
                ...
                if manifest:
                    ...
                    try:
                        ...
                        if plugin and hasattr(plugin, 'on_install'):
                            # 获取数据库文件路径
                            from modules.inspection.dal import DEFAULT_DB_PATH
    ...
    def _install_from_local(self, plugin_id: str, source_path: str, target_dir: str) -> Dict:
        ...
        try:
            ...
            # 动态加载（先加载到 available/，并捕获真实失败原因）
            from modules.pluginkit.core import load_plugin_with_error
            manifest, load_err = load_plugin_with_error(target_dir)
            if manifest:
                logger.info(f"插件安装成功（available/）: {plugin_id}")
                # 自动启用：复制到 enabled/
