...
def _run_mariadb():
    ...
    import modules.entrypoints.main_mariadb as main_mariadb
    main_mariadb.main()


def _run_oceanbase():
    """启动 OceanBase（MySQL 租户）数据库巡检（复用 main_oceanbase 逻辑）"""
    import modules.entrypoints.main_oceanbase as main_oceanbase
    main_oceanbase.main()
...
def _run_yashandb():
    import modules.entrypoints.main_yashandb as main_yashandb
    main_yashandb.main()
...
def _run_gbase():
    """启动 GBase 8s 数据库巡检"""
    import modules.entrypoints.main_gbase as main_gbase
    main_gbase.main()
...
def _run_web_ui():
    """启动 Web UI"""
    from i18n import t
    import modules.web.app as web_ui
