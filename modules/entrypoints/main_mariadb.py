...
使用方式：
    from modules.entrypoints.main_mariadb import MariaDBInspector
...
class MariaDBInspector(BaseInspectionEngine):
    ...
    def connect(self):
        """
        连接 MariaDB 数据库（统一 JDBC 优先，回退 pymysql）。
        ...
        try:
            ...
            _conn, _meta = open_jdbc_connection(
                'mariadb', self.host, int(self.port),
                user=self.user, password=self.password,
                database=self.database or 'mysql',
                driver_version=self.driver_version,
                fallback_dirs=[_os.path.join(str(PROJECT_ROOT), 'drivers', 'mariadb')],
            )
            if _conn is None:
    ...
    def _connect_native(self, reason=''):
        ...
        try:
            ...
            self.cursor.execute("SELECT VERSION()")
            ver = self.cursor.fetchone()[0]
            print(f"[MariaDB] pymysql 回退连接成功（JDBC: {reason[:80]}...）" if reason else "[MariaDB] pymysql 连接成功")
            return True, ver
...
def getData(ip, port, user, password, ssh_info=None, template_id=None, database=None, driver_version=''):
    ...
    """
    inspector = MariaDBInspector(ip, port, user, password, database, ssh_info, template_id, driver_version)
    ok, ver = inspector.connect()
    if not ok:
