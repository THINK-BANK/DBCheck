...
"""
SQL Server JDBC 共享 JVM 管理模块（单例）。

职责：
  - 在项目根目录 drivers/**/*.jar 中一次性收集所有 JDBC 驱动 jar；
  - 首次调用 ensure_jvm() 时启动 JVM，并将全部驱动 jar 加入 classpath；
  - 若 JVM 已由其它插件（如 db2_jdbc / oracle_jdbc）先行启动，则通过反射把缺失
    的 jar 动态追加到系统类加载器的 classpath，无需重启 JVM；
  - 提供统一的 MS SQL Server 驱动注册入口，供 main_plugin 在 connect() 时调用。

设计要点（与 db2_jdbc/jdbc_jvm.py 一致，仅 Class.forName 的类名不同）：
  ...
  - 模块独立性：本文件不复用 db2_jdbc.jdbc_jvm，避免强耦合 + 同名模块抢注
    （参见 db2_jdbc/main_plugin._load_own_jdbc_jvm 的设计说明）。
  - 进程级共享：两份 jdbc_jvm.py 共享同一 JVM 进程（jpype.isJVMStarted() 守护）。
"""
...
# 项目根目录：本文件位于 <root>/plugins/available/sqlserver_jdbc/jdbc_jvm.py
# 向上三级（sqlserver_jdbc -> available -> plugins -> root）即项目根
...
def _require_jars_on_classpath(jars: List[str]) -> None:
    ...
    以 MS JDBC 13.4 为例：若 com.microsoft.sqlserver.jdbc.SQLServerDriver
    可加载，说明驱动 jar 已在 startJVM 时通过 classpath 参数注入，无需动作；
    否则给出明确错误（需改用 JDK 8，或在 JVM 首次启动时就由 ensure_jvm() 注
    入全部驱动 jar）。
    """
    try:
        JClass("com.microsoft.sqlserver.jdbc.SQLServerDriver")
    except Exception:
        raise RuntimeError(
            "JVM 已启动但未能在 classpath 中找到 MS SQL Server 驱动"
            "（com.microsoft.sqlserver.jdbc.SQLServerDriver），且当前 JDK 不支持运行时 addURL。"
...
def _append_to_classpath(jars: List[str]) -> None:
    ...
    for jar in jars:
        ...
        added += 1
    if added:
        print(f"[sqlserver_jdbc.jvm] 已动态追加 {added} 个 jar 到运行中的 JVM classpath")
    ...
    if not jpype.isJVMStarted():
        if not all_jars:
            raise RuntimeError(
                ...
                "请确认 drivers/sqlserver/mssql-jdbc-13.4.0.jre11.jar 等驱动文件已就位。"
            )
        jpype.startJVM(classpath=all_jars, convertStrings=True)
        print(f"[sqlserver_jdbc.jvm] JVM 已启动，classpath 含 {len(all_jars)} 个 jar")
    else:
...
def register_mssql_driver() -> None:
    """向 DriverManager 注册 MS SQL Server JDBC 驱动（幂等）。

    需在 ensure_jvm() 之后调用。重复注册无害。

    Raises:
        RuntimeError: 当 JVM 未启动或驱动类不可加载时。
    """
    if not jpype.isJVMStarted():
        raise RuntimeError("register_mssql_driver() 调用前必须先 ensure_jvm()")
    # 触发驱动类静态初始化（JDBC 4 下 DriverManager 也会自动发现，
    # 此处显式加载以保证驱动一定就绪）。
    try:
        JClass("java.lang.Class").forName("com.microsoft.sqlserver.jdbc.SQLServerDriver")
    except Exception as e:
        raise RuntimeError(
            "MS SQL Server 驱动类 com.microsoft.sqlserver.jdbc.SQLServerDriver "
            f"不可加载（{e}）。请确认 mssql-jdbc-*.jar 已放入 drivers/sqlserver/。"
        ) from e
...
__all__ = ["ensure_jvm", "register_mssql_driver", "get_driver_jars"]
