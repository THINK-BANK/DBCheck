...
# 项目根目录：本文件位于 <root>/plugins/available/uxdb_jdbc/jdbc_jvm.py
# 向上三级（uxdb_jdbc -> available -> plugins -> root）即项目根
...
def _require_jars_on_classpath(jars: List[str]) -> None:
    ...
    以 UXDB 为例：若 com.uxsino.uxdb.Driver 可加载，说明驱动 jar 已在
    startJVM 时通过 classpath 参数注入，无需动作；否则给出明确错误（需改用
    JDK 8，或在 JVM 首次启动时就由 ensure_jvm() 注入全部驱动 jar）。
    """
    try:
        JClass("com.uxsino.uxdb.Driver")
    except Exception:
        raise RuntimeError(
            "JVM 已启动但未能在 classpath 中找到 UXDB 驱动"
            "（com.uxsino.uxdb.Driver），且当前 JDK 不支持运行时 addURL。"
    ...
    if not jpype.isJVMStarted():
        if not all_jars:
            raise RuntimeError(
                ...
                "请确认 drivers/uxdb/uxdbjdbc-*.jar 等驱动文件已就位。"
            )
        jpype.startJVM(classpath=all_jars, convertStrings=True)
        print(f"[uxdb_jdbc.jvm] JVM 已启动，classpath 含 {len(all_jars)} 个 jar")
    else:
...
def register_uxdb_driver() -> None:
    """向 DriverManager 注册 UXDB JDBC 驱动（幂等）。

    需在 ensure_jvm() 之后调用。重复注册无害。
    """
    if not jpype.isJVMStarted():
        raise RuntimeError("register_uxdb_driver() 调用前必须先 ensure_jvm()")
    # 触发驱动类静态初始化（JDBC 4 下 DriverManager 也会自动发现，
    # 此处显式加载以保证驱动一定就绪）。
    JClass("java.lang.Class").forName("com.uxsino.uxdb.Driver")
...
__all__ = ["ensure_jvm", "register_uxdb_driver", "get_driver_jars"]
