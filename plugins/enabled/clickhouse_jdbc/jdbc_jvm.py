...
"""ClickHouse JDBC 共享 JVM 管理模块（单例）。

职责（与 db2_jdbc 同款，仅驱动注册函数改为 ClickHouse）：
  - 在项目根目录 drivers/**/*.jar 中一次性收集所有 JDBC 驱动 jar；
  - 首次调用 ensure_jvm() 时启动 JVM，并将全部驱动 jar 加入 classpath；
  - 若 JVM 已由其它插件（如 oracle_jdbc / db2_jdbc）先行启动，则通过反射把
    缺失的 jar 动态追加到系统类加载器的 classpath，无需重启 JVM；
  - 提供统一的 ClickHouse 驱动注册入口，供 main_plugin 在 connect() 时调用。

设计要点：
  - 单一事实来源：所有 JDBC 驱动统一放在 <root>/drivers/ 下，ensure_jvm()
    用 glob 一次性收集，避免各插件单独指定 jar 导致冲突。
  - JVM 进程级单例：jpype.isJVMStarted() 守护，重复调用 ensure_jvm() 安全。
  - 目标 JDK 为 1.8（plugin.json min_java_version=1.8）：AppClassLoader 为
    URLClassLoader，反射 addURL 追加 classpath 可行。

注意：clickhouse-jdbc*.jar 及其传递依赖（clickhouse-http-client / httpclient5 /
jackson / slf4j 等）需由用户自行放入 <root>/drivers/clickhouse/，本模块只负责
在 ensure_jvm() 时统一收集，不会在仓库内提交任何 *.jar 二进制。
"""
...
# 项目根目录：本文件位于 <root>/plugins/available/clickhouse_jdbc/jdbc_jvm.py
# 向上三级（clickhouse_jdbc -> available -> plugins -> root）即项目根
...
def _require_jars_on_classpath(jars: List[str]) -> None:
    ...
    以 clickhouse-jdbc 为例：若 com.clickhouse.jdbc.Driver 可加载，说明驱动 jar
    已在 startJVM 时通过 classpath 参数注入，无需动作；否则给出明确错误（需改用
    JDK 8，或在 JVM 首次启动时就由 ensure_jvm() 注入全部驱动 jar）。
    """
    try:
        JClass("com.clickhouse.jdbc.Driver")
    except Exception:
        raise RuntimeError(
            "JVM 已启动但未能在 classpath 中找到 ClickHouse 驱动"
            "（com.clickhouse.jdbc.Driver），且当前 JDK 不支持运行时 addURL。"
            "请使用 JDK 8，或在 JVM 首次启动时通过 ensure_jvm() 注入驱动 jar，"
            "并把 clickhouse-jdbc*.jar 放到 drivers/clickhouse/ 目录。"
        )
...
def _append_to_classpath(jars: List[str]) -> None:
    ...
    for jar in jars:
        ...
        added += 1
    if added:
        print(f"[clickhouse_jdbc.jvm] 已动态追加 {added} 个 jar 到运行中的 JVM classpath")
    ...
    if not jpype.isJVMStarted():
        if not all_jars:
            raise RuntimeError(
                ...
                "请确认 drivers/clickhouse/clickhouse-jdbc*.jar 等驱动文件已就位。"
            )
        jpype.startJVM(classpath=all_jars, convertStrings=True)
        print(f"[clickhouse_jdbc.jvm] JVM 已启动，classpath 含 {len(all_jars)} 个 jar")
    else:
...
def register_clickhouse_driver() -> None:
    """向 DriverManager 注册 ClickHouse JDBC 驱动（幂等）。

    需在 ensure_jvm() 之后调用。重复注册无害。
    """
    if not jpype.isJVMStarted():
        raise RuntimeError("register_clickhouse_driver() 调用前必须先 ensure_jvm()")
    # 触发驱动类静态初始化（JDBC 4 下 DriverManager 也会自动发现，
    # 此处显式加载以保证驱动一定就绪）。
    JClass("java.lang.Class").forName("com.clickhouse.jdbc.Driver")
...
__all__ = ["ensure_jvm", "register_clickhouse_driver", "get_driver_jars"]
