...
@dataclass
class UxdbConnectionConfig:
    """UXDB JDBC 连接配置。

    Attributes:
        host: UXDB 服务器主机名或 IP
        port: UXDB 实例端口（默认 33060）
        user: 登录用户名（默认 uxdb）
        password: 登录密码
        database: 目标数据库名（默认 uxdb）
        jdbc_url: 完整 JDBC URL（可选；以 jdbc:uxdb 开头则直接透传，不做拼接）
    """
    ...
    def build_jdbc_url(self) -> str:
        ...
        - 若 jdbc_url 以 'jdbc:uxdb' 开头：原样透传（支持自定义属性）
        - 否则按标准格式拼接： jdbc:uxdb://{host}:{port}/{database}

        Returns:
            可用的 JDBC 连接 URL 字符串
        """
        if self.jdbc_url and str(self.jdbc_url).strip().lower().startswith("jdbc:uxdb"):
        ...
        return _build_jdbc_url('uxdb', self.host, self.port, database=self.database)
    ...
    @classmethod
    def from_instance(cls, inst: dict) -> "UxdbConnectionConfig":
        ...
        return cls(
            ...
            port=_get_int("port", 33060) or 33060,
            user=_get_str("user", "uxdb") or "uxdb",
            password=_get_str("password", ""),
            database=_get_str("database", "uxdb") or "uxdb",
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"UxdbConnectionConfig(host={self.host!r}, port={self.port}, "
